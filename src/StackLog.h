#pragma once

#include <cinttypes>
#include <fstream>
#include <iostream>
#include <vector>

#include "Stack.h"

class StackLog {
public:
  StackLog();

  // No need to close the files, the destructor of ofstream does it.
  ~StackLog();

  // Creates the output files containing the stacks.
  bool init(std::size_t len, std::size_t minwr, std::size_t maxwr,
            uint64_t frac, uint64_t modeq);

  void log_stack(const Stack &s, std::size_t dst);
  void log_starting_stack();
  void log_tried_candidate(int start);

  void write_stack_counts(int start);
  void write_stats(uint64_t flip_cnt, int maxheapsize,
                   uint64_t greedo_checks = 0);

private:
  std::size_t _len;
  std::size_t _minwr;
  std::size_t _maxwr;
  uint64_t _frac;
  uint64_t _modeq;
  uint64_t _gencnt{0};   // number of stacks of size START generated so far
  uint64_t _triescnt{0}; // number of all tried stacks
  uint64_t _fact[MAX_SIZE + 2]; // factorial

  uint64_t _visstackcnt[MAX_SIZE + 2][MAX_STEP + 2]; // data for .scnt file
  std::size_t _diam[MAX_SIZE + 2]; // diam[i] = maximum number of flips among
                                   // tried stacks of size i

  std::vector<std::ofstream> _outf;
};
